import React, { useState, useRef,MutableRefObject,FC,ReactInstance ,ChangeEvent } from 'react'
...
const CartModal:FC<Props>=({ DecreaseCartProduct,removeCartProduct,totalAmount,cart, addProductToCart})=> {
  ...
  const handleInputChange = (event: ChangeEvent<HTMLInputElement> ,val:number) => {
    if (val==taxValue){
        ...
        <div className="col-lg-12">
          ...
          <div>
            <table className='table table-responsive table-secondary table-hover'>
              ...
              <tbody>
                {cart ? cart.map((cartProduct: any, key: any) =>
                  <tr key={key}>
                    ...
                    <td>
                      ...
                      {cartProduct.quantity > 1 ?
                        <button className='btn btn-light btn-sm bg-light text-black ' onClick={() => DecreaseCartProduct(cartProduct)}>-</button>
                        :
                    ...
                    <td>
                      <button className='btn btn-danger btn-sm ' onClick={() => removeCartProduct(cartProduct)} ><MdDeleteOutline className='mb-1' /></button>
                    </td>
                  </tr>)
              ...
              <table className='table border  '>
                ...
                <tbody>
                  <tr className='border'>Added Tax
                    <td><input type="number" name="tax" className='w-25 border' value={taxValue} onChange={(e)=>handleInputChange(e,taxValue)} /></td>
                  </tr>
                  <tr className='border'>Discount
                    <td><input type="number" name="discount" className=' w-25 border' value={discountValue} onChange={(e)=>handleInputChange(e,discountValue)} /></td>
                  </tr>
